...
type DeepReading = { english: string; chinese: string; life: string; lens: string; human: string; takeAway: string };
...
const details = (id: number): Omit<Chapter, "id" | "part"> => {
  if (id <= 4) return {
    focus: "西西里乡村与童年", background: "1900 年前后的西西里：贫困、佃农关系与父权家庭塑造了一个孩子可拥有的选择。注意身体与劳动描写如何提示阶级位置。", read: "先连续阅读英文原文，不逐词翻译；只标记妨碍理解、反复出现或很有表达力的词块。读完后对照中文翻译检查理解。", bilingual: ["Class is shown through work, space, and who gets to speak.", "阶级通过劳动、空间，以及谁有资格发言来呈现。"], analysis: "从具体的家庭和空间关系入手：谁拥有资源，谁只能承受后果？"
  };
  if (id <= 18) return {
    focus: "修道院、教育与规训", background: "convent 不只是宗教场所，也是教育、慈善和纪律机构。理解称谓（nun、Mother、priest）背后的等级，比记住全部宗教词更重要。", read: "先连续阅读英文原文，不逐词翻译；优先注意与权威、服从、身体或空间有关的表达。读完后对照中文翻译检查理解。", bilingual: ["Care and control can exist in the same institution.", "照护与控制可以存在于同一个制度中。"], analysis: "观察善意、保护与限制是否同时出现；不要急着把人物简化为好人或坏人。"
  };
  if (id <= 29) return {
    focus: "庄园、贵族与社会流动", background: "estate、title、inheritance 和 marriage 构成贵族权力的语言。西西里的土地租佃链也让佣人、管理者和地主之间的关系很复杂。", read: "先连续阅读英文原文。阅读后在词汇本记下一个「身份词」、一个「财产词」和一个「关系动词」。读完后对照中文翻译检查理解。", bilingual: ["A title can carry power, duty, and constraint at once.", "一个头衔可以同时携带权力、义务与束缚。"], analysis: "本章可从「私人关系是否也被财产和家名塑造」这一问题进入。"
  };
  if (id <= 39) return {
    focus: "战争前夕与家族秩序", background: "1915 年意大利参加第一次世界大战。乡村生活与国家大事并不同步，但战争、价格、劳动力和家族荣誉会逐步进入私人生活。", read: "先连续阅读英文原文。遇到长句时圈出主语、核心动词和转折词，再看修饰语。读完后对照中文翻译检查理解。", bilingual: ["History enters private life through small changes in power and language.", "历史通过权力与语言中的细小变化进入私人生活。"], analysis: "留意时间推进：社会变化如何改变人物原以为固定的规则？"
  };
  if (id <= 57) return {
    focus: "成年、亲密关系与后果", background: "1920 年代的婚姻、继承、名誉与母职仍强烈约束女性。注意 diary、rumour、memory 等信息来源：叙述不总是完全可靠。", read: "先连续阅读英文原文。只把真正想再次使用的表达加入词汇卡，并用一句自己的英文造句。读完后对照中文翻译检查理解。", bilingual: ["A choice can be personal and still be shaped by social rules.", "一个选择可以是个人的，同时仍被社会规则塑造。"], analysis: "将人物的选择与其可得到的资源分开看：她真正拥有多少选择？"
  };
  if (id <= 74) return {
    focus: "法西斯时期与新的思想", background: "1922 年后法西斯逐步建立一党统治。自由主义、社会主义、无政府主义与共产主义并存，角色的公开用语可能与私人立场不同。", read: "先连续阅读英文原文。标记一个政治词、一个情绪词和一个连接词（however / therefore / although 等）。读完后对照中文翻译检查理解。", bilingual: ["Public language and private belief are not always the same.", "公开语言与私人信念并不总是一致。"], analysis: "先问「谁在说、对谁说、在什么风险下说」，再判断政治观点。"
  };
  if (id <= 84) return {
    focus: "战争、流离与国家权力", background: "二战时期，旅行、艺术教育、家庭和生育都受不稳定环境影响。法西斯国家的监控、逮捕与监禁会改变人们说话和沉默的方式。", read: "先连续阅读英文原文。若内容令人不适，可只写一条中性事实摘要，并停止精读细节。读完后对照中文翻译检查理解。", bilingual: ["Silence can be a response to fear, grief, or power.", "沉默可以是对恐惧、悲伤或权力的回应。"], analysis: "观察宏大政治如何通过家庭、身体和日常对话被感受到。"
  };
  return {
    focus: "解放后、政治与记忆", background: "1943 年政权垮台、1945 年解放后，重建并不等于问题消失。战后左翼政党、工会、家庭代际冲突与政治幻灭共同构成新的环境。", read: "先连续阅读英文原文。今天写 60–100 词英文日志：What changed, and why does it matter? 读完后对照中文翻译检查理解。", bilingual: ["Liberation is a process, not a single finished moment.", "解放是一个过程，而不是一个瞬间完成的事件。"], analysis: "将自由、快乐、权力和历史放在一起思考；结尾尤其适合回看全书早期的选择。"
  };
};
...
type ChapterText = { en: string; zh: string };
...
export default function Home() {
  ...
  const [readerMessage, setReaderMessage] = useState("点击下方按钮，选择你下载好的 EPUB 文件即可开始阅读。");
  ...
  // Load chapter text from JSON files
  useEffect(() => {
    let cancelled = false;
    async function load() {
      setTextLoading(true);
      setChapterText(null);
      try {
        const id = String(chapterId).padStart(3, '0');
        const res = await fetch(`/chapters/ch${id}.json`);
        if (!res.ok) throw new Error('not found');
        const data = await res.json();
        if (!cancelled) setChapterText(data);
      } catch {
        if (!cancelled) setChapterText({ en: `Chapter ${chapterId} text loading...`, zh: '' });
      } finally {
  ...
  // Auto-load EPUB from localStorage
  useEffect(() => {
    const savedBook = localStorage.getItem("joy-epub-name");
    if (savedBook) {
      setReaderMessage(`上次阅读：${savedBook}。重新选择文件以继续。`);
    }
  }, []);
  ...
  async function openEpub(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      localStorage.setItem("joy-epub-name", file.name);
      setReaderMessage("正在阅读：" + file.name.replace(/\.epub$/i, ""));
    } catch {
  ...
  return (
    <main>
      ...
      <section className="today-card">
        ...
        {/* READING: English original text */}
        <section className="reading-text">
          <h3>阅读 · Chapter {chapter.id}</h3>
          {textLoading ? (
            <p className="loading-msg">正在加载章节内容…</p>
          ) : chapterText ? (
            <>
              {chapterText.en && chapterText.en !== `Chapter ${chapterId} text loading...` ? (
                <div className="english-text">{chapterText.en}</div>
              ) : (
                <div className="fallback-epub">
                  <p className="reading-hint">{chapter.read}</p>
                  <label className="import-button">
                    选择本地 EPUB
                    <input ref={readerInputRef} type="file" accept="application/epub+zip,.epub" onChange={openEpub} />
                  </label>
                  <p className="reader-message">{readerMessage}</p>
                  <div ref={readerRef} className="epub-canvas" />
                </div>
              )}
            </>
          ) : (
        ...
        {chapterText?.zh && (
          <div className="zh-toggle">
            <button className="text-btn" onClick={() => setShowZh(!showZh)}>
              {showZh ? "隐藏中文翻译 ▲" : "显示中文翻译 ▼"}
            </button>
            {showZh && <div className="zh-text">{chapterText.zh}</div>}
          </div>
        )}
        ...
        <section className="after-reading">
          ...
          <div className="after-grid">
            <article>
              ...
              <small>这是基于本章主题的学习摘要，帮助你对照理解。</small>
            </article>
            <article>
              <h3>章节结束 · 赏析</h3>
          ...
          <details className="research-basis">
            ...
            <ul>
              <li><a href="https://www.treccani.it/enciclopedia/goliarda-sapienza_%28Dizionario-Biografico%29/" target="_blank" rel="noreferrer">Treccani · Goliarda Sapienza 传记</a></li>
              <li><a href="https://www.researchgate.net/publication/310526424_L%27arte_della_gioia_di_Goliarda_Sapienza_tra_anarchia_e_femminismo_postmoderno" target="_blank" rel="noreferrer">Dell&apos;Abate Çelebi (2016)</a></li>
              <li><a href="https://rosa.uniroma1.it/rosa03/status_quaestionis/article/view/18794" target="_blank" rel="noreferrer">Status Quaestionis</a></li>
              <li><a href="https://revistas.usp.br/italianistica/en/article/view/223698" target="_blank" rel="noreferrer">Revista de Italianística</a></li>
            </ul>
          </details>
